...
<script>
//위에 include먼저 읽고 밑에 실행.
//다 읽혀지기 전이기때문에 어쩔수없이 window.onload사용
//로딩이 다 된 다음에 호출하게되는 시점이다.
	window.onload=function(){
		summernote_go($('textarea#content'));
	}
	
	function regist_go(){
		var form = document.registForm; //해당 form태그에 regist_go로 되있기때문에 해당 dom은 해당 name으로 찾아감.
		if(form.title.value==""){
			alert("제목은 필수입니다.");
