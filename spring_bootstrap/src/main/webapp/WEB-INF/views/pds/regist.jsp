...
<script>
	window.onload=function(){
		...
		//외부에서 가져와야하기때문에 핸들러를 어쩔수없이 써야한다.
		//핸들러에서 this안쓰면 외부에서 가져오기 힘들어짐.
		$('.fileInput').on('change','input[type="file"]',function(event){
			//lert(this.files[0].size);
...
function addFile_go(){
	...
	//갯수제한이 필요. 선택자 잡아서 length로 잡음. 배열로 들어옴.
	...
	//자바스크립트.(옆에 x버튼만듦. String으로 이렇게 만드는건 좋은 방법이 아님. 이렇게 하지 말것)
	div.append(input).append("<button onclick='remove_go("+dataNum+");' style='border:0;outline:0;' class='badge bg-red' type='button'>X</button>");
	$('.fileInput').append(div);
	
	//function콜은 마크업찍을때부터 준비를 하고있어야함. 버튼의 인자와 지우려고하는인자를 알 수 있게 해야한다.
	dataNum++;
