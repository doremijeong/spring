...
<body>
					...
					<div class="card-footer" style="padding: 5px 0;">
						<div class="row">
							...
							<div class="col-sm-3 text-center">
								<button type="button" onclick="location.href='modifyForm.do?id=${member.id}';" id="modifyBtn"
									<%-- style="display: ${loginUser.id eq member.id ? 'visible':'none'};" --%> class="btn btn-warning ">수 정</button>
							</div>
							<div class="col-sm-3 text-center">
								<button type="button" onclick="location.href='remove.do?id=${member.id}';" id="deleteBtn"
									class="btn btn-danger">삭 제</button>
							</div>
							<div class="col-sm-3 text-center">
								<c:if test="${member.enabled ne 0 }">
									<button type="button" onclick="location.href='enabled.do?id=${member.id}&enabled=0';" id="stopBtn"
										class="btn btn-info">비활성</button>
								</c:if>
								<c:if test="${member.enabled eq 0 }">
									<button type="button" onclick="location.href='enabled.do?id=${member.id}&enabled=1';" id="activeBtn"
	...
	<script>
		//common.js가 읽혀진 다음에 읽어야해 window.onload를 주었음.
		//자바스크립트 객체이기때문에  jquery로 선택자 잡으려면 [0]번지이고, $로 감싸야함. 자바스크립트의 dom은 모든 요소를 접근할수 있다.
		window.onload=function(){ 
			MemberPictureThumb(document.querySelector('[data-id="${member.id}"]'),'${member.picture}','<%=request.getContextPath()%>');
			
			//el문은 null이 찍히지이않아서 에러가 찍힘. 그래서 if안에 el문 사용하면 안됨.
			//if(${parentReload})
